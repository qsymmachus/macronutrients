...
describe "NutritionGetter, an abstract class for retrieving nutrition data" do
  it "completes a food nutrition search and build a FoodNutrition object" do
    apple_nutrition = NutritionGetter.first_result("apples")
    expect(apple_nutrition.class).to eq FoodNutrition
  end

  it "completes a food nutrition search for the first x results" do
  ...
  it "returns false if a food does not exist" do
    expect(NutritionGetter.food_exists?("asdfasdf")).to eq false
  end

  it "parses a nutrition description into a hash of macros" do
  ...
  it "returns false if a food is not in the database" do
    expect(NutritionGetter.food_in_local_database?("awergih")).to eq false
  end

  it "returns true if a food is in the database" do
  ...
  it "adds food to the database" do
    ...
    expect(test_pear.food_nutritions).to_not be_empty
  end
end